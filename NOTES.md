...
### Етап 14: Adding Authentication

**Що зроблено:**
Додано систему авторизації для захисту маршрутів дашборду:
1. Встановлено та налаштовано бібліотеку `NextAuth.js`.
2. Створено файл `auth.config.ts` та налаштовано **Middleware** (проміжне програмне забезпечення). Тепер неавторизовані користувачі, які намагаються зайти на `/dashboard`, автоматично перенаправляються на сторінку `/login`.
3. Налаштовано провайдер `Credentials` у файлі `auth.ts`. Реалізовано логіку перевірки email та пароля з бази даних (з використанням `bcrypt` для порівняння хешів паролів).
4. Оновлено клієнтський компонент `LoginForm`, підключено Server Action `authenticate` та хук `useActionState` для виведення помилок (наприклад, "Invalid credentials").
5. Додано функціонал виходу з системи (Sign Out).

**Скріншот:**
![Login Form](./screenshots/loginForm.png)

**Мої думки/питання:**
Використання Middleware для захисту сторінок це дуже потужно. Перевірка доступу відбувається ще до того, як сторінка почне рендеритись на сервері. Також `NextAuth.js` бере на себе всю складну роботу з управління сесіями та кукі-файлами.
### Етап 15: Adding Metadata

**Що зроблено:**
Налаштовано метадані застосунку для покращення SEO (Search Engine Optimization) та відображення при поширенні в соцмережах:
1. Додано об'єкт `metadata` у кореневий `layout.tsx` із загальним заголовком (title) та описом (description).
2. Використано об'єкт `title.template` у кореневому макеті (`%s | Acme Dashboard`), що дозволяє автоматично підставляти назву конкретної сторінки у формат заголовку.
3. Додано специфічні метадані на вкладені сторінки (наприклад, сторінку Invoices), щоб кожна сторінка мала свій унікальний `title`.

**Скріншот:**
![Page Metadata](./screenshots/metadata.png)

**Мої думки/питання:**
Next.js робить роботу з `<head>` і метатегами дуже зручною. Замість того, щоб вручну прописувати теги на кожній сторінці, можна задати шаблон у Layout, і всі вкладені сторінки будуть його успадковувати та доповнювати.